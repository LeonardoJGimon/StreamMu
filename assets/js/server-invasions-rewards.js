(function () {
    ...
    const PAGE_CONFIG = {
        invasion_rewards: { title: 'Invasions Rewards', file: 'invasions.json' }
    };

    const MENU_CHUNK_SIZE = 80;
    ...
    function normalizeSlug(entry) {
        const idx = Number(entry?.monsterIndex ?? -1);
        const name = String(entry?.monsterName ?? '').trim().toLowerCase();
        const safeName = name.replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
        return `${idx}-${safeName || 'entry'}`;
    }
    ...
    function buildMenuItem(entry, slug) {
        const label = entry?.monsterName ? String(entry.monsterName) : `Entry ${entry?.monsterIndex ?? ''}`;
        return `<a href="#inv-${escapeHtml(slug)}" data-slug="${escapeHtml(slug)}">${escapeHtml(label)}</a>`;
    }

    function renderMenuChunked(nav, entries, onSelect) {
        if (!nav) return;
        ...
        function appendChunk() {
            const end = Math.min(index + MENU_CHUNK_SIZE, entries.length);
            ...
            for (; index < end; index++) {
                ...
                wrapper.innerHTML = buildMenuItem(entry, slug);

                const link = wrapper.firstChild;
                link.addEventListener('click', (e) => {
                    e.preventDefault();
                    onSelect(slug, entry);
                    // keep hash in URL so refresh preserves selection
                    location.hash = `inv-${slug}`;
                });
    ...
    function buildIndex(entries) {
        ...
        for (let i = 0; i < entries.length; i++) {
            ...
            map.set(normalizeSlug(entry), entry);
        }
        return map;
    }

    // ---------- Rendering helpers (same as your bosses version) ----------
    ...
    function itemTooltipHtml(item) {
        ...
        if (isExcellentItem(item)) lines.push(excText(item?.exc));
        if (isSocketItem(item)) lines.push(socketText(item?.socketCount));
    ...
    function shouldShowCountBadge(item) {
        // only for yellow “normal” items; never for exc/socket
    ...
    function getOrCreateTooltip() {
        const existing = document.querySelector('.br-tooltip');
    ...
    function renderEntryDetails(root, entry) {
        if (!root) return;

        const coinName = entry?.coins?.name ?? 'Unknown';
        const coinAmount = Number(entry?.coins?.amount ?? 0);
        const coinEnabled = Boolean(entry?.coins?.enabled);

        const ruudChance = Number(entry?.ruud?.chancePercent ?? 0);
        const ruudMin = Number(entry?.ruud?.min ?? 0);
        const ruudMax = Number(entry?.ruud?.max ?? 0);
      ...
      <div class="section-card">
        <div class="si-bar"><span>${escapeHtml(entry?.monsterName ?? 'Invasion')}</span></div>
    ...
    async function init() {
        ...
        try {
            const entries = await fetchServerJson(cfg.file);
            ...
            entries.sort((a, b) => String(a?.monsterName ?? '').localeCompare(String(b?.monsterName ?? '')));
            const entryBySlug = buildIndex(entries);

            const onSelect = (slug, entry) => {
                setActiveMenuSlug(nav, slug);
                renderEntryDetails(root, entry);
            };

            renderMenuChunked(nav, entries, onSelect);

            // initial selection: hash > first entry
            const hash = String(location.hash || '');
            const match = /^#inv-(.+)$/.exec(hash);

            const initialSlug = match?.[1] || normalizeSlug(entries[0]);
            const initialEntry = entryBySlug.get(initialSlug) || entries[0];

            setActiveMenuSlug(nav, normalizeSlug(initialEntry));
            renderEntryDetails(root, initialEntry);
        } catch (e) {
