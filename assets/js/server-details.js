(function () {
    ...
    function renderDonationJourney() {
        const steps = [
            { icon: 'download', title: t('vip_journey.step1_title', 'DESCARGA EL LAUNCHER'), desc: t('vip_journey.step1_desc', 'Obtén el Launcher Oficial desde nuestra sección de descargas.') },
            { icon: 'user-plus', title: t('vip_journey.step2_title', 'ABRE E INICIA SESIÓN'), desc: t('vip_journey.step2_desc', 'Crea tu cuenta en la web o directamente desde el Launcher.') },
            { icon: 'wallet', title: t('vip_journey.step3_title', 'DONACIÓN'), desc: t('vip_journey.step3_desc', 'Entra al Launcher y selecciona Donaciones en el panel lateral.') },
            { icon: 'crown', title: t('vip_journey.step4_title', 'COMPRA'), desc: t('vip_journey.step4_desc', 'Elige tu paquete, completa el pago y ¡listo!') }
        ];

        const stepsHtml = steps.map((s, idx) => `
            <div class="hi-card">
                <div class="hi-num-row">
                    <div class="hi-num">0${idx + 1}</div>
                    <div class="hi-icon"><i class="fas fa-${s.icon}"></i></div>
                </div>
                <div class="hi-title" data-i18n="vip_journey.step${idx + 1}_title">${escapeHtml(s.title)}</div>
                <div class="hi-desc" data-i18n="vip_journey.step${idx + 1}_desc">${escapeHtml(s.desc)}</div>
            </div>
        `).join('');
        ...
        return `
            <div class="hi-wrapper">
                <div class="hi-header">
                    <i class="fas fa-crown"></i> <span data-i18n="vip_journey.title">${escapeHtml(t('vip_journey.title', 'CÓMO ADQUIRIR TU VIP'))}</span>
                </div>
                <div class="hi-grid">${stepsHtml}</div>
            </div>
        `;
    }
    ...
    function renderVipSystem(vip) {
        ...
        const html = tiers.map(t => {
            ...
            const isGold = name.toLowerCase().includes('oro') || name.toLowerCase().includes('gold');
            
            let iconFile = 'bronze.png';
            if (name.toLowerCase().includes('plata') || name.toLowerCase().includes('silver')) {
                iconFile = 'plata.png';
            } else if (isGold) {
                iconFile = 'gold.png';
            }
            ...
            return `
                <div class="vip-tier ${isGold ? 'vip-tier--gold' : ''}">
                    <div class="vip-tier-icon-wrap">
                        <img src="/assets/icon/${iconFile}" class="vip-tier-img" alt="${escapeHtml(name)} icon">
                    </div>
                    <div class="vip-tier-title">
                        <span>${escapeHtml(name)}</span>
                        ${isGold ? '<span class="vip-badge-anim">RECOMMEND</span>' : ''}
                    </div>
        ...
        return `<div class="vip-tiers-wrapper">${html}</div>${renderDonationJourney()}`;
    }
    ...
    function section(title, icon, bodyHtml) {
        ...
        return `
            <section id="sec-${escapeHtml(slug)}" class="section-card" data-slug="${escapeHtml(slug)}" data-label="${escapeHtml(title)}" data-icon="${escapeHtml(icon)}" data-collapsed="1">
    ...
    function buildMenuFromSections(root) {
        ...
        const sections = Array.from(root.querySelectorAll('.section-card[data-slug]'))
            .map(s => ({
                ...
                icon: s.getAttribute('data-icon') || 'chevron-right',
        ...
        if (nav) {
            nav.innerHTML = sections.map(x => `
                <a href="#sec-${escapeHtml(x.slug)}" data-slug="${escapeHtml(x.slug)}">
                    <div class="sm-left"><i class="fas fa-${escapeHtml(x.icon)} fa-fw"></i> <span>${escapeHtml(x.label)}</span></div>
                    <i class="fas fa-chevron-right sm-chevron"></i>
                </a>
            `).join('');
            nav.querySelectorAll('a[data-slug]').forEach(a => {
    ...
    async function init() {
        ...
        try {
            ...
            try { if (typeof applyTranslations === 'function') applyTranslations(root); } catch(e) {}
            bindAccordion(root);
            bindScrollSpy(root);
